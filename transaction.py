    ...
    def parse_transaction(hex_data):
        ...
        for i in range(len(fields)):
            if fields[i] is 'txIns':
                txIns, offset = TransactionParser.parse_txIns(field_vals['input count'], hex_data[start_pos:])
                field_vals[fields[i]] = txIns
                start_pos = start_pos + offset
            elif fields[i] is 'txOuts':
                txOuts, offset = TransactionParser.parse_txOuts(field_vals['output count'], hex_data[start_pos:])
